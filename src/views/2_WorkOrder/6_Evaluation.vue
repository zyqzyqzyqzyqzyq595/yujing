...
const getWorkOrderStatsRecords = () => {
  try {
    const raw = localStorage.getItem(WORKORDER_STATS_KEY);
    if (!raw) return [];

    const parsed = JSON.parse(raw);
    return Array.isArray(parsed.records) ? parsed.records : [];
  } catch (error) {
    console.error('读取工单统计源失败：', error);
    return [];
  }
};
const syncFalseAlarmStatsFromWorkOrder = () => {
  const records = getWorkOrderStatsRecords();

  const level12Records = records.filter(item =>
      item && ['red', 'orange', 'yellow', 'blue'].includes(item.levelClass)
  );

  const falseAlarmRecords = level12Records.filter(item =>
      item.supervisorDecision === 'false_alarm'
  );

  falseEventList.value = falseAlarmRecords.map(item => ({
    id: item.id || '',
    time: item.supervisorTime || item.time || '',
    region: item.area || '',
    position: item.area || '',
    recorder: item.supervisorName || item.staff || '',
    description: item.supervisorOpinion || item.opinion || '主管判定为误报',
    photoName: item.processInfo?.attachmentName || item.archiveInfo?.attachmentName || '未上传'
  }));

  const total = level12Records.length;
  const falseCount = falseAlarmRecords.length;

  kpiData.value.falseRate = total > 0
      ? Number(((falseCount / total) * 100).toFixed(1))
      : 0;
};
